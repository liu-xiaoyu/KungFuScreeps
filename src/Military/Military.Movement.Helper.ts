...
export class MilitaryMovement_Helper {
     ...
     */
    public chooseRallyLocation(fromRoomName: string, targetRoomName: string, rallyOpts: RallyOpts) {
        ...
        for (let i = routeToRoom.length - 1; i >= 0; i++) {
            ...
            // TODO Check if the current room is elegible to rally in
                // * Might include the check above in this method
        ...
        if(rallyPosition === undefined) {
            throw new UserException("Error in chooseRallyLocation", "Could not find a suitable area to rally, even inside of the creeps current room.", ERROR_INFO);
        }
     ...
     */
    public getOpenPositionNearExit(roomName: string, exit: ExitConstant): RoomPosition | undefined {
        ...
        let exitX = exit === FIND_EXIT_LEFT ? 0 : FIND_EXIT_RIGHT ? 49 : undefined;
        let exitY = exit === FIND_EXIT_TOP ? 0 : FIND_EXIT_BOTTOM ? 49 : undefined;
        ...
        let xOffset = exit === FIND_EXIT_LEFT ? 3 : FIND_EXIT_RIGHT ? -3 : 0;
        let yOffset = exit === FIND_EXIT_TOP ? -3 : FIND_EXIT_BOTTOM ? 3 : 0;

        let testRallyPosition = new RoomPosition(exitRange[exitPositionIndex].x + xOffset, exitRange[exitPositionIndex].y + yOffset, roomName);

        // TODO Get the position to rally in if testRallyPosition is a wall
    }
     ...
     */
    public getOpenTilesInRow(roomName:string, xRow?: number, yRow?: number): RoomPosition[]  {

        if(!(xRow && !yRow) || (xRow && yRow) ){
            throw new UserException("Invalid arguments for getOpenTilesInRow()", "Either xRow or yRow should be defined, but not both.", ERROR_INFO);
        }
     ...
     */
    public getRoomInDirection(roomName: string, exit: ExitConstant): string {
        ...
        if(exit === FIND_EXIT_TOP) {
            return parsedName[2] === "N" ? this.offsetRoomName(roomName, 0, 1) : this.offsetRoomName(roomName, 0, -1);
        } else if(exit === FIND_EXIT_BOTTOM) {
            return parsedName[2] === "N" ? this.offsetRoomName(roomName, 0, -1) : this.offsetRoomName(roomName, 0, 1);
        } else if(exit === FIND_EXIT_LEFT) {
            return parsedName[0] === "W" ? this.offsetRoomName(roomName, 1, 0) : this.offsetRoomName(roomName, -1, 0);
        } else { // FIND_EXIT_RIGHT
            return parsedName[0] === "W" ? this.offsetRoomName(roomName, -1, 0) : this.offsetRoomName(roomName, 1, 0);
        }
     ...
     */
    public offsetRoomName(roomName: string, xOffset: number, yOffset: number): string {
        const parsedName: RegExpExecArray | null = /^([WE])([0-9]+)([NS])([0-9]+)$/.exec(roomName);
        ...
        adjustedName[0] = parsedName[0] as string;
        adjustedName[1] = parseInt(parsedName[1], 10) + xOffset;
        adjustedName[2] = parsedName[2];
        adjustedName[3] = parseInt(parsedName[3], 10) + yOffset;

        // If x went negative, switch directions and make it positive
        if (adjustedName[1] < 0) {
            adjustedName[1] = adjustedName[1] * -1;
            adjustedName[0] = parsedName[0] === "W" ? "E" : "W";
        }

        // If y went negative, switch directions and make it positive
        if (adjustedName[3] < 0) {
            adjustedName[3] = adjustedName[3] * -1;
            adjustedName[2] = parsedName[2] === "N" ? "S" : "N";
        }
